...
public:
	void Execute( const shaderreg *i_pInput, vector4 &o_vPosition, shaderreg *o_pOutput )
	{
		...
		vWaves *= 2.0f * M3D_PI;   //M3D_PI = 3.1415926  vWaves = vWaves*2*pi
		...
		// apply deformation in direction of normal using sine-waves 使用正弦函数对顶点进行法线方向的变换__猜的
		...
		// pass texcoord to pixel shader 纹理坐标
...
//pBubble->bInitialize( 20.0f, 16, 16 )
bool CBubble::bInitialize( float32 i_fRadius, uint32 i_iStacks, uint32 i_iSlices )
{
	...
	// Construct a sphere
	m_iNumVertices = i_iStacks * i_iSlices * 4;    // 16*16*4 = 1024 顶点数量
	m_iNumPrimitives = i_iStacks * i_iSlices * 2;  // 512 片元数量
	...
	if( FUNC_FAILED( m_pVertexBuffer->GetPointer( 0, (void **)&pDestVertices ) ) ) // 0是offSet 
		return false;
	...
	const float32 fStepV = 1.0f / (float32)i_iStacks;  //  1/16
	const float32 fStepU = 1.0f / (float32)i_iSlices;  //  1/16
	 
	uint32 iCurVertex = 0;

	float32 fV = 0.0f;  //球坐标系统 球上采样点的坐标位置  fV,fU是纹理坐标
	for( uint32 i = 0; i < i_iStacks; ++i, fV += fStepV )  // 循环16次，fV从0.0开始，每次步进1/16
	{
		float32 fNextV = fV + fStepV;

		float32 fU = 0.0f;
		for( uint32 j = 0; j < i_iSlices; ++j, fU += fStepU )  //循环16次
		{
			...
			x[0] = i_fRadius * sinf( fV * M3D_PI ) * cosf( fU * 2.0f * M3D_PI );  // M3D_PI=3.1415926
...
void CBubble::Render( uint32 i_iPass )
{
	switch( i_iPass )
	{
	case ePass_Default: break;    // i_iPass等于多少都没啥用。。。 ePass_Default=0
	}
	...
	matrix44 matWorld; 
	matMatrix44Identity( matWorld );  // 将matWorld变为单位矩阵
	...
	pGraphics->SetRenderState( m3drs_cullmode, m3dcull_cw );  //顺时针 
	// m3dpt_trianglelist(枚举类型) 每次绘制都会传入所有的顶点数量和图元数量
	pGraphics->pGetM3DDevice()->DrawIndexedPrimitive( m3dpt_trianglelist,
		0, 0, m_iNumVertices, 0, m_iNumPrimitives );

	pGraphics->SetRenderState( m3drs_cullmode, m3dcull_ccw ); // 逆时针
	pGraphics->pGetM3DDevice()->DrawIndexedPrimitive( m3dpt_trianglelist,
		0, 0, m_iNumVertices, 0, m_iNumPrimitives );
}
