...
result CMuli3DPresentTargetWin32::Create()
{
	...
	{        //create an instance of a DirectDraw object that supports the set of D3D interfaces
	...
	if( DeviceParameters.bWindowed )
	{
		...
		// Create clipper 创建裁减器(指定目标页面的特定区域是可写的)
...
{  //计算i_iMask中从低往高数第一个为1的位，记为i_iMask，从该位开始，计算位为1的个数，记为o_iNumBits
	uint32 iTestMask = 1;
