...
result CMuli3DDevice::SetVertexFormat( CMuli3DVertexFormat *i_pVertexFormat )
{
	...
	for( uint32 iReg = 0; iReg < c_iVertexShaderRegisters; ++iReg )
		m_RenderInfo.VSInputs[iReg] = m3dsrt_unused;     // m3dsrt_unused = 0
...
{  // CBubble中iOffset=0,iStride=sizeof(vertexformat),i_iStreamNumber=0
	if( i_iStreamNumber >= c_iMaxVertexStreams )   // c_iMaxVertexStreams=8
	{
...
result CMuli3DDevice::SampleTexture( vector4 &o_vColor, uint32 i_iSamplerNumber, float32 i_fU, float32 i_fV, float32 i_fW, const vector4 *i_pXGradient, const vector4 *i_pYGradient )
{
	...
	case m3dtsi_2coords:
		...
		{
		case m3dta_wrap: i_fU -= ftol( i_fU );               // 测试发现有四舍五入的意思 则 i_fU在 -0.5和0.5之间 
		case m3dta_clamp: i_fU = fSaturate( i_fU ); break;   // i_fU在0 1之间则返回原值 小于0则返回0 大于1则返回1
...
result CMuli3DDevice::SetScissorRect( const m3drect &i_ScissorRect )
{
	...
	m_RenderInfo.ScissorPlanes[0] = plane(  1,  0,  0, -(float32)m_ScissorRect.iLeft ); //iLeft等等这几个数都是非负数
...
{   //pM3DDevice->CreateSurface( &pColorBuffer, i_iWidth, i_iHeight, i_fmtFrameBuffer ) framebuffer
	//pM3DDevice->CreateSurface( &pDepthBuffer, i_iWidth, i_iHeight, m3dfmt_r32f ) depthbuffer
	if( !o_ppSurface )
	{
...
{
	const byte *pVertex[c_iMaxVertexStreams];   // 顶点流 max=8
	const vertexstream *pCurVertexStream = m_VertexStreams;
	for( uint32 iStream = 0; iStream <= m_pVertexFormat->iGetHighestStream(); ++iStream, ++pCurVertexStream )
	{
	...
	while( iElement-- )
	{
		...
		case m3dvet_float32:
			...
			Register = shaderreg( pData[0], 0, 0, 1 );     //typedef vector4 shaderreg 
			break;
		case m3dvet_vector2:
...
result CMuli3DDevice::FetchVertex( m3dvertexcacheentry **io_ppVertex, uint32 i_iVertex )
{
	...
	const bool bCanAppend = ( m_iNumValidCacheEntries < c_iVertexCacheSize );   // c_iVertexCacheSize = 32
	...
	for( uint32 iCurEntry = 0; iCurEntry < m_iNumValidCacheEntries; ++iCurEntry, ++pCacheEntry )
	{
		if( pCacheEntry->iVertexIndex == i_iVertex )
		{
			...
			pCacheEntry->iFetchTime = m_iFetchedVertices++;   // m_iFetchedVertices记录当前获取的顶点的个数 
...
{ // 计算点A(i_pVSOutputA)和点B(i_pVSOutputB)按比例i_fInterpolation得到的插值点o_pVSOutput
	...
	for( uint32 iReg = 0; iReg < c_iPixelShaderRegisters; ++iReg, ++pO, ++pA, ++pB )  // c_iPixelShaderRegisters=8
	{
...
{
	if( io_pVSOutput->vPosition.w < FLT_EPSILON )   //  FLT_EPSILON can be regarded as the smallest number bigger than 0
		return;
...
inline bool CMuli3DDevice::bCullTriangle( const m3dvsoutput *i_pVSOutput0, const m3dvsoutput *i_pVSOutput1, const m3dvsoutput *i_pVSOutput2 )
{
	...
	if( m_iRenderStates[m3drs_cullmode] == m3dcull_ccw )  // 逆时针剔除
	{
		if( fDirTest <= 0.0f )
			return true;
	}
...
{  //iClipToPlane( iNumVertices(3), iStage(0or1), m_RenderInfo.ClippingPlanes[iPlane], true );
	m3dvsoutput **ppSrcVertices = m_pClipVertices[i_iSrcIndex];
	m3dvsoutput **ppDestVertices = m_pClipVertices[(i_iSrcIndex + 1) & 1];   // i_iSrcIndex为奇数则结果为0 ，偶数则为1 m_pClipVertices[]也只有2个，两个数组交替
	...
	for( uint32 i = 0, j = 1; i < i_iNumVertices; ++i, ++j )
	{
		...
		if( i_bHomogenous )   // class plane的乘法(vect3有vect3的乘法，vec4有vec4的乘法)
		{
			di = i_plane * ppSrcVertices[i]->vPosition;  // 可被视为计算点到面的距离
...
{
	// Prepare triangle for homogenous clipping ------------------------------- 齐次的裁剪
	uint32 iNumVertices = 3;
	memcpy( &m_ClipVertices[0], i_pVSOutput0, sizeof( m3dvsoutput ) );  //m3dvsoutput m_ClipVertices[20];
	...
	uint32 iStage = 0;
	m_pClipVertices[iStage][0] = &m_ClipVertices[0];    // m3dvsoutput *m_pClipVertices[2][20];
	...
	// Perform clipping to the frustum planes --------------------------------- 锥面裁剪
	...
	// Project and rasterize the clipped triangle ----------------------------- 投影和光栅化 裁剪 三角形
	uint32 iVertex;
