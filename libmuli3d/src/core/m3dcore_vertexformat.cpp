		...
		uint32 i_iVertexDeclSize )
{
	...
	m_iNumVertexElements = i_iVertexDeclSize / sizeof( m3dvertexelement );  // 2
	m_iHighestStream = 0;
	...
	{
		if( pCurElement->iStream >= c_iMaxVertexStreams )   // c_iMaxVertexStreams=8
		{
			FUNC_FAILING( "CMuli3DVertexFormat::Create: vertex element's stream number exceeds number of available streams.\n" );
			return e_invalidparameters;
		}

		if( pCurElement->iRegister >= c_iVertexShaderRegisters )  // c_iVertexShaderRegisters=8
		{
