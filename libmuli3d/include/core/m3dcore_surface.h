...
public:
	...
	// Samples the surface using bi-linear filtering.(二线性过滤/插补 它是通过在原材质中对不同像素间利用差值算法的柔化处理来平滑图像的。其工作是以目标纹理的像素点为中心，它会找出该点最近像素附近的4个图素（texel），然后在它们之间作差补效果，然后再将这个平均颜色值贴至目标图像素的位置上)
	// @param[out] o_vColor receives the color of the pixel to be looked up.
	// @param[in] i_fU u-component of the lookup-vector.
	// @param[in] i_fV v-component of the lookup-vector.
